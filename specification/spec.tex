...
\subsection{Data reference transformations}
\begin{figure}
\begin{center}
...
\npar The pipeline pool is the entry point for a configured pipeline into the system.
...
\npar We would like to maintain a library of pipelines (\glspl{abstract pipeline}) which can be used to allow pipeline runners (particularly outside of HGI) to select one of the standard analyses to be run over their data. This is the pipeline catalogue.
In general, we do not see the pipeline catalogue as storing the definitive record of pipelines - since the identity of the pipeline is a vital component of the provenance record, we envision this being stored in the central metadata system. Instead, the catalogue would provide the ability for people to select popular pipelines which have been registered with the catalogue. An identity would then be constructed by the pipeline executor from the pipeline description and registered with the metadata system.
\npar The workflow for the pipeline catalogue can be thought of as such:
\begin{enumerate}
\item User searches for, or selects, a specific pipeline.
\item User configures the pipeline for specific inputs.
\item The configured pipeline is submitted into the execution queue.
\end{enumerate}
\npar The pipeline catalogue exists much in isolation to the rest of the system, and as such can definitely be thought of as an optional component. It should be possible to submit pipelines However, it may well prove the major interface to the system for a number of users, so when created some thought should be put into presentation and usability.
...
\item Any assertions (see \ref{sec:generalisedAssertions}) made about steps or pipelines.
\end{itemize}
